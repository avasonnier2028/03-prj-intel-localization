...
- (Bonus) Auto-detect language switches from LTR to RTL
    -Implemented JavaScript script to detect language changes and dynamically apply RTL mode



Adapt for RTL and Localization: Use Bootstrap to enable right-to-left text flow.

Three-Column Section: Use Bootstrap’s grid system to structure a new three-column layout that adapts to different screen sizes & style the content
  -Integrate icons to visually enhance each column heading.
  -Style the "Learn More" links in each column.

Add a Subscription Form: Use Bootstrap to build a simple form for subscribing to Intel’s sustainability newsletter.
...
Improve Accessibility: Run accessibility tests with Lighthouse to detect issues.
  -Adjust color contrast and ensure descriptive alt attributes are present for all images.
  -Ensure the form follows proper accessibility and usability guidelines.
